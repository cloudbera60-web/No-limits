...
function serialize(m, sock, logger) {
    ...
    sock.sendContact = async (jid, kon, quoted = '', opts = {}) => {
        let list = [];
        for (let i of kon) {
            let name = config.OWNER_NAME || 'Owner';
            list.push({
                displayName: name,
    ...
    sock.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
        ...
        // Fix: Use data.length instead of file.length
        ...
        if (options.asSticker || /webp/.test(mime)) {
            // Use imported writeExif functions instead of require
            if (/image/.test(mime)) {
                pathFile = await writeExifImg(data, { 
                    packname: options.packname || 'Sticker', 
                    author: options.author || '', 
                    categories: options.categories || [] 
                });
            } else if (/video/.test(mime)) {
                pathFile = await writeExifVid(data, { 
                    packname: options.packname || 'Sticker', 
                    author: options.author || '', 
                    categories: options.categories || [] 
                });
    ...
    sock.copyNForward = async (jid, message, forceForward = false, options = {}) => {
        ...
        const waMessage = await generateWAMessage(jid, content, options ? {
            ...content[ctype],
            ...options,
            ...(options.contextInfo ? {
                contextInfo: {
    ...
    if (m.message) {
        ...
        m.messageTypes = type => ["videoMessage", "imageMessage", "stickerMessage", "audioMessage", "documentMessage"].includes(type);
